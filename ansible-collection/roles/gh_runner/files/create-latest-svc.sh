...
if [ -d ./runner ]; then
    runner_exists "Runner already exists.  Use a different directory or uninstall and delete existing runner"
fi
...
latest_version_label=$(curl -s -X GET 'https://api.github.com/repos/actions/runner/releases/latest' -H "authorization: token ${RUNNER_CFG_PAT}" | jq -r '.tag_name')
